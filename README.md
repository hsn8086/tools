...
- **便签生成器** (`/note`)：把短文本和短问题做成图片，支持六种版式、三种比例、字体与署名调整、自动缩字和本地草稿。出处可选「同款入口」「简洁」或关闭，默认附带可单独关闭的二维码，扫码进入对应风格。二维码在浏览器内生成，不含正文；复制模板链接可分享完整排版参数。运行 `pnpm test:note` 检查排版、导出图压缩后的二维码识别、草稿和移动端。
...
便签「湖畔来信」的照片来自 [Unsplash](https://images.unsplash.com/photo-1470770841072-f978cf4d019e)，遵循 [Unsplash License](https://unsplash.com/license)。图片存放于 `public/assets/note/lake.jpg`，导出时无需访问外部图片服务。
...
- **导出与字体就绪**：使用 `html-to-image` 截取各工具的卡片容器，配合 `pixelRatio` 生成 2x / 3x 清晰度图片。截图前等待 `document.fonts.ready`，避免字体回退导致文字跳变。
